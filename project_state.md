...
| 31 | Signature: single app-wide image (`SignatureStore`, `signature.image` in App Support, plain data), drawn in-app (transparent PNG, stroke-cropped) or imported PNG/JPEG; new `signature` field type whose session value REUSES `.checkbox(Bool)` ("signed?"); image drawn aspect-fit in preview + export; included in library backups (add-only on restore, like templates); erased by Reset App | User request modeled on EYEreport. Reusing the checkbox value case keeps FieldValue/CodableFieldValue/payload untouched — old drafts and exported payloads stay decodable. The signature is the user's own info, not PHI, so plain storage + backup inclusion is fine | 2026-07-18 |
| 32 | New editor fields arrive with their placeholder name focused and fully selected (`fieldAwaitingName` one-shot in the editor VM + iOS 18 `TextField(selection:)`) | User request: type the real name immediately, no select-all fiddling | 2026-07-18 |
| 33 | Reopen-exported-PDF pushes detail-then-fill onto the navigation path, and the restored session is vaulted to the draft immediately | Back now lands on the fill/edit chooser (matching the normal flow) and backing out can never lose a reopened form — the resume prompt offers it on the next fill entry | 2026-07-18 |
| 35 | Signature rolled into practitioner profiles: `PractitionerProfile.signatureBase64` (rides in `practitioners.json` + backups automatically); draw/import/remove UI moved into the profile form; standalone Settings Signature section removed; fill session stamps the SELECTED profile's signature, falling back to the legacy app-wide `SignatureStore` image; profile picker also shows for templates with only signature fields (`usesProfileSelection`) | One doctor, multiple locations = per-profile signatures; switching profile switches signature. Legacy fallback keeps pre-profile setups working with zero migration; `SignatureStore` retained solely as that fallback (still cleared by Reset App, still exported/restored in backups for old files) | 2026-07-18 |
| 34 | Practitioner profiles (`PractitionerProfile` + `practitioners.json`): six auto-populated field types (doctorName/officeAddress/officeFax/officePhone/officeEmail/practitionerID) **materialized into `values` as plain `.text`** when the session starts / profile changes; hidden from the fill form; `officeAddress` wraps (`FieldType.isMultiline`); picker in the fill form when >1 profile; profiles in backups (add-only by ID); `hasAnyValues` EXCLUDES practitioner values | Materializing means preview/export/draft/embedded-payload need zero special cases, and a reopened PDF shows exactly what was printed. Excluding them from hasAnyValues keeps untouched forms from autosaving drafts or enabling "Clear form" | 2026-07-18 |
...
- **2026-07-18** — Round 2 confirmed working (after two MemberImportVisibility fixes: CoreGraphics in FillSessionViewModel, UniformTypeIdentifiers in SettingsView). Round 3 written, NOT yet built: **Signature** — `Services/SignatureStore.swift`, `Views/Settings/SignatureDrawingView.swift` (Canvas strokes → transparent stroke-cropped PNG), Settings "Signature" section (draw / import PNG-JPEG / preview / remove), `FieldType.signature` (session value reuses `.checkbox(Bool)`), aspect-fit drawing in `FillFieldOverlay` + `PDFExportService.drawSignature`, `signatureBase64` in `LibraryBackup` (add-only restore), Reset App clears it (Decision #31). **Editor** — new fields get name focused + selected via `fieldAwaitingName` + `TextField(selection:)` (Decision #32; verify the select-all actually lands on device — focus timing under Form is the risky bit). **Reopen fix** — detail-then-fill push + immediate draft vault of the restored session (Decision #33). Docs updated (user renamed them to `docs/Form-Filler-user-manual.md` / `Form-Filler-privacy-policy.md`; internal link fixed). Note: a reopened PDF silently overwrites any existing draft for a *different* template — deemed acceptable, flag if wrong.
- **2026-07-18 (b)** — Practitioner profiles (round 4), NOT yet built. New: `Models/PractitionerProfile.swift` (defensive Codable, `value(for:)`), `Services/PractitionerStore.swift` (`practitioners.json`, atomic), `Views/Settings/PractitionerProfilesView.swift` (list + add/edit form sheet, swipe-delete, Save requires a name); Settings gained a "Practitioner Profiles" NavigationLink. Six new FieldTypes (Decision #34) with icons + inspector footer; `FieldType.isPractitionerField`/`isMultiline` (all `== .multiLineText` layout checks now go through isMultiline). FillSessionViewModel: loads profiles, `selectProfile(id:)` re-materializes, `clearAll` repopulates profile fields, `hasAnyValues` counts only user entries. FillFormListView: practitioner section (picker when >1 profile, Settings hint when 0). Backup: `practitioners` array (defensive decode, add-only merge by ID); Reset App clears the store. 2 new tests (backup carry + store round-trip); docs updated. Verify: picker switching updates the preview live; profile fields excluded from Clear-form enablement; backup from an old build (no practitioners key) still restores.
- **2026-07-18 (c)** — Profile labels: `PractitionerProfile.label` (defensive decode, default "") — the picker/list name, independent of the printed doctor name, so one doctor can keep per-location profiles ("Dr Smith — Downtown"). `displayLabel` prefers label, falls back to doctor name; profile form gained an optional "Profile name" field; list rows show the doctor name as subtitle when a distinct label hides it. Old saved profiles/backups decode unchanged.
- **2026-07-18 (d)** — Signature rolled into profiles (Decision #35), NOT yet built. `PractitionerProfile` gained `signatureBase64` + `signatureImage` (defensive decode; needed `import UIKit` in the model); profile form gained the Signature section (draw sheet, PNG/JPEG import, preview, remove — edits are local to the form until Save); SettingsView's standalone Signature section and helpers deleted; `FillSessionViewModel.signatureImage` now derives from the selected profile with legacy `SignatureStore` fallback, refreshed by `selectProfile`; picker section condition switched to `usesProfileSelection`; hints reworded; docs updated. Verify: toggling a signed field then switching profiles restamps the new signature in preview AND export; a pre-profile signature (legacy store) still stamps when profiles lack one; profile-form Cancel discards a drawn signature.
- **2026-07-18 (e)** — White field backgrounds, NOT yet built. `FieldDefinition.whiteBackground: Bool?` (defensive decode; nil = per-type default via `fillsWhiteBackground` — ON for multiLineText, off otherwise, so existing templates get it "by default" with no migration); drawn only when the field has content, identically in export (`UIGraphicsGetCurrentContext` fill before text in `drawFieldText`) and preview (`Color.white` layer in `FillFieldOverlay`, also behind signatures on fields toggled on); inspector "White Background" toggle in Text Style (explicit choice sticks across type changes). Verify: filled multi-line box hides ruled lines in preview AND exported PDF; empty fields never paint white; toggle off restores transparency.
