...
struct LibraryView: View {
    ...
    @State private var importReadyAcknowledged = false
    ...
    var body: some View {
        ...
        .modifier(PatientImportAlerts(
            acknowledged: $importReadyAcknowledged,
            atRoot: navigationPath.isEmpty
        ))
...
/// Alerts for a PDF shared into the app while at the library: a "ready"
/// prompt (only at the root — inside a fill session the details are applied
/// directly) and a parse-failure message. Factored into a modifier so the
/// library's already-large body stays within the type-checker's budget.
private struct PatientImportAlerts: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .alert("Couldn't Read PDF", isPresented: failureBinding) {
                Button("OK", role: .cancel) { importInbox.failureMessage = nil }
            } message: {
                Text(importInbox.failureMessage ?? "")
            }
            .alert("Patient Details Ready", isPresented: readyBinding) {
                Button("OK") { acknowledged = true }
            } message: {
                Text("Details for \(importInbox.pending?.fullName ?? "the patient") are ready. Open the form you want to fill and they'll appear for you to review.")
            }
            .onChange(of: importInbox.pending) { _, newValue in
                if newValue != nil { acknowledged = false }   // a new import: prompt again
            }
    }

    private var readyBinding: Binding<Bool> {
        Binding(
            get: { importInbox.pending != nil && atRoot && !acknowledged },
            set: { if !$0 { acknowledged = true } }
        )
    }
...
#Preview {
    LibraryView()
        .environment(PatientImportInbox())
}
