...
nonisolated struct PDFExportService: Sendable {
        ...
        let data = renderer.pdfData { rendererContext in
            for pageIndex in 0..<document.pageCount {
                ...
                for field in template.fields where field.pageIndex == pageIndex {
                    if let text = FieldValueFormatting.displayText(for: field, value: values[template.valueKey(for: field)]) {
    ...
    // MARK: - Template-share copy

    /// A clean Core Graphics re-render of `document` carrying `keywords` in
    /// its document info. Used to embed a shareable template payload into
    /// PDFs whose tag structure defeats PDFKit's dataRepresentation()
    /// ("mixing the old and new tagging functions"). Rendering from scratch
    /// drops the structure tree, so the result carries Keywords reliably;
    /// geometry is preserved for ordinary un-rotated, origin-0 forms (the
    /// common case for tagged referral PDFs).
    func pdfCopy(ofSource document: PDFDocument, keywords: String) -> Data {
        let format = UIGraphicsPDFRendererFormat()
        format.documentInfo = [
            kCGPDFContextCreator as String: "Form Filler",
        ...
        return ensuringKeywords(data, keywords: keywords)
    }

    /// Verifies `keywords` reads back from `data`; if a render path dropped
    /// it, re-serializes ONCE through PDFKit. The data is already a clean,
    /// untagged CG render, so this can't hit the tagging conflict.
    private func ensuringKeywords(_ data: Data, keywords: String) -> Data {
        let target = keywords.trimmingCharacters(in: .whitespacesAndNewlines)
        func present(in candidate: Data) -> Bool {
            Self.keywordsCandidates(in: candidate)
                .contains { $0.trimmingCharacters(in: .whitespacesAndNewlines) == target }
        }
        if present(in: data) { return data }
        guard let document = PDFDocument(data: data) else { return data }
        var attributes = document.documentAttributes ?? [:]
        attributes[PDFDocumentAttribute.keywordsAttribute] = keywords
        document.documentAttributes = attributes
        guard let rewritten = document.dataRepresentation(), present(in: rewritten) else { return data }
        return rewritten
    }
    ...
    private func drawFieldText(_ text: String, field: FieldDefinition, space: PageCoordinateSpace) {
        ...
        switch field.type {
        case .multiLineText, .officeAddress, .patientAddress:
            attributed.draw(with: rect, options: options, context: nil)
        case .singleLineText, .staticText, .patientName, .signature, .referralDate,
