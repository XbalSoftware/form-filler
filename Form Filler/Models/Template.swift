...
nonisolated struct Template: Codable, Identifiable, Equatable, Sendable {
    ...
    /// The primary patient-name field (first in fill order). Its value feeds
    /// the export filename and is mirrored onto any other patient-name
    /// fields (e.g. the same name repeated on page 2).
    var patientNameField: FieldDefinition? {
        orderedFields.first { $0.type == .patientName }
    }

    /// The values-dictionary key a field reads from. Every patient-name
    /// field mirrors the primary one, so a name placed on multiple pages
    /// shares a single stored value; all other fields use their own id.
    func valueKey(for field: FieldDefinition) -> UUID {
        guard field.type == .patientName else { return field.id }
        return patientNameField?.id ?? field.id
    }
}
