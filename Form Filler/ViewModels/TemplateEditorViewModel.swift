...
final class TemplateEditorViewModel {
    ...
    func selectAdjacentField(offset: Int) -> Bool {
        ...
        select(field)
        // Only named fields get a name-focus request; self-labeling fields
        // have no name box (the inspector's Tab-catcher takes focus there).
        fieldAwaitingName = field.type.isSelfLabeling ? nil : field.id
        return true
    }
    ...
    /// Changes the selected field's type. Self-labeling types (patient,
    /// practitioner, signature) carry their meaning in the type itself, so
    /// the name is set to echo the type — no redundant typing, and the
    /// inspector then hides the name box.
    func setSelectedFieldType(_ newType: FieldType) {
        updateSelectedField { field in
            field.type = newType
            if newType.isSelfLabeling {
                field.name = newType.displayName
            }
        }
    }
