...
final class FillSessionViewModel {
    ...
    /// in the form. (Referral date DOES appear — it's editable, defaulting
    /// to today.)
    var formFields: [FieldDefinition] {
        template.orderedFields.filter { field in
            guard field.type != .staticText, field.type != .signature,
                  !field.type.isPractitionerField else { return false }
            // Mirrored patient-name fields share one input: show only the primary.
            if field.type == .patientName { return field.id == template.patientNameField?.id }
            return true
        }
    }
    ...
    var keyboardNavigableFields: [FieldDefinition] {
        formFields.filter {
            $0.type == .singleLineText || $0.type == .multiLineText
                || $0.type.isPatientField || $0.type == .referralDate
        }
    }
    ...
    func displayText(for field: FieldDefinition) -> String? {
        FieldValueFormatting.displayText(for: field, value: values[template.valueKey(for: field)])
    }
    ...
    func handleOverlayTap(_ field: FieldDefinition) {
        ...
        case .patientName:
            // Any mirrored name field focuses the single (primary) input.
            focusDidChange(to: template.valueKey(for: field))
        case .singleLineText, .multiLineText, .referralDate,
    ...
    /// True when the template has any patient field an import can fill.
    var hasPatientFields: Bool {
        template.fields.contains { $0.type.isPatientField }
    }

    /// Parses a picked/shared PDF into the patient details this template can
    /// actually use (limited to the patient field types it contains). nil
    /// when the file can't be read or nothing usable was found — the caller
    /// shows the review sheet only for a non-nil result.
    func patientDemographics(fromPDFAt url: URL) -> PatientDemographics? {
        let didStartAccess = url.startAccessingSecurityScopedResource()
        defer { if didStartAccess { url.stopAccessingSecurityScopedResource() } }
        guard let parsed = PatientDemographicsParser.demographics(fromPDFAt: url) else { return nil }
        return reviewableDemographics(parsed)
    }

    /// Trims already-parsed demographics (e.g. from a shared PDF) to the
    /// patient field types this template actually has; nil if nothing would
    /// populate. Used by the share-in flow, which parses before a session
    /// is known.
    func reviewableDemographics(_ demographics: PatientDemographics) -> PatientDemographics? {
        let usable = demographics.limited(to: Set(template.fields.map(\.type)))
        return usable.isEmpty ? nil : usable
    }

    /// Writes confirmed demographics into the matching patient fields and
    /// autosaves the draft. Only fields the import has a value for change;
    /// others (and every non-patient field) are left untouched.
    func applyDemographics(_ demographics: PatientDemographics) {
        for field in template.fields where field.type.isPatientField {
            guard let value = demographics.value(for: field.type)?
                .trimmingCharacters(in: .whitespacesAndNewlines), !value.isEmpty
            else { continue }
            values[template.valueKey(for: field)] = .text(value)
        }
        autosaveDraft()
    }
