    ...
    Template(
        ...
        fields: [
            ...
            FieldDefinition(
                ...
                pageIndex: 0,
                rect: CGRect(x: 60, y: 500, width: 160, height: 20),
                sortOrder: 3
            ),
            FieldDefinition(
                name: "Clinic Header",
...
struct TemplateCodableTests {
    ...
    @Test func patientNameFieldsMirrorThePrimary() {
        let name1 = FieldDefinition(name: "Patient Name", type: .patientName, pageIndex: 0, rect: .zero, sortOrder: 0)
        let name2 = FieldDefinition(name: "Patient Name", type: .patientName, pageIndex: 1, rect: .zero, sortOrder: 5)
        let other = FieldDefinition(name: "AHC", type: .patientHealthcareNumber, pageIndex: 0, rect: .zero, sortOrder: 1)
        var template = Template(name: "Two Page")
        template.fields = [name1, name2, other]
        // Both name fields read from the first one; other fields use their own id.
        #expect(template.valueKey(for: name1) == name1.id)
        #expect(template.valueKey(for: name2) == name1.id)
        #expect(template.valueKey(for: other) == other.id)
    }
}
